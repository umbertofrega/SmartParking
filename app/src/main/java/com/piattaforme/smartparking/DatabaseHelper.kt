...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, "history.db", null, 1) {
    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL("CREATE TABLE history (_id INTEGER PRIMARY KEY AUTOINCREMENT, latitude FLOAT, longitude FLOAT);")
    }
    ...
    fun insertParking(latitude : Float, longitude: Float): Boolean {
